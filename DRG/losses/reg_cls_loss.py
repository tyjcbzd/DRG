...
class LossFunc(nn.Module):
    ...
    def forward(self, out: Dict[str, List[torch.Tensor]], gt_preds: List[torch.Tensor], pad_flags: List[torch.Tensor]): 
        cls = out[0] 
        reg = out[1] 
        ...
        device = gt_preds.device
        
        loss_out = dict()
        num_modes = self.config["g_num_modes"]
        num_preds = 30
        
        last = has_preds.float() + 0.1 * torch.arange(num_preds).float().to(device) / float(num_preds)
        ...
        row_idcs = torch.arange(len(last_idcs)).long().to(device)
        dist = [] 
        ...
        row_idcs = torch.arange(len(min_idcs)).long().to(device) 
